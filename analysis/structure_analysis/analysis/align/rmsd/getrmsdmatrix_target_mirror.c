...
double **rmsdmatrix,***coor,***targetcoor,***negtargetcoor,temprmsd;
...
negtargetcoor = doublematrixarray(filenum,targetstatenum,DIM);
...
for(i=0;i<filenum;i++)
 for(j=0;j<targetstatenum;j++)
  for(k=0;k<DIM;k++)
   ...
   negtargetcoor[i][j][k] = -targetcoor[i][j][k];
   }
...
for(i=0;i<filenum;i++)
 for(j=0;j<filenum;j++)
  ...
  if(i <= j)
   ...
   temprmsd = getrmsd(targetcoor[i],negtargetcoor[j],targetstatenum);
   if(rmsdmatrix[i][j] > temprmsd)
    rmsdmatrix[i][j] = temprmsd;
//   temprmsd = 
   }
  else
...
for(i=0;i<filenum;i++)
 ...
 for(j=0;j<targetstatenum;j++)
  {
  free(targetcoor[i][j]);
  free(negtargetcoor[i][j]);
  }
 free(targetcoor[i]);
 free(negtargetcoor[i]);
 }
free(rmsdmatrix);
free(coor);
free(targetcoor);
