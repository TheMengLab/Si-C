...
double ***coor,*avedist,**distmatrix,maxvalue;
...
for(i=0;i<intervalnum;i++)
 ...
 for(j=0;j<intervalnum;j++)
  {
//  if((assign[targetid[i]][2] == -1)||(assign[targetid[j]][2] == -1))
//   fprintf(outputfile,"-100 ");
//  else if(i==j)
  if(i==j)
   fprintf(outputfile,"%lf ",maxvalue);
  else 
   fprintf(outputfile,"%lf ",pow(distmatrix[i][j]*2,-3)*maxvalue);
//log(distmatrix[i][j]*2)/log(10));
//   fprintf(outputfile,"%lf ",log(distmatrix[i][j]*2)/log(10));
