...
double getcov(double *data1list,double *data2list,int statenum,double ave1,double ave2,int targetnum,int *assign)
 {
 int i,j;
 double cov;
...
double **matrix,**corrmatrix,*avelist,*eigen_real,*eigen_image,**leftmatrix,tempmean;
...
eigen_real = doublearray(statenum);
eigen_image = doublearray(statenum);
...
for(i=0;i<statenum;i++)
 ...
 for(j=0;j<statenum;j++)
  {
  if(i<=j)
   corrmatrix[i][j] = getcov(matrix[i],matrix[j],statenum,avelist[i],avelist[j],targetnum,assign);
  else
...
geteigen(corrmatrix,statenum,eigen_real,eigen_image,leftmatrix);
geteigen_sort_dec(eigen_real,statenum,leftmatrix);
...
for(i=0;i<statenum;i++)
 if(assign[i] != -1)
  {
  if(leftmatrix[0][i] > tempmean)
   fprintf(assignfile,"%d %d %lf\n",i,1,leftmatrix[0][i]);
  else
   fprintf(assignfile,"%d %d %lf\n",i,0,leftmatrix[0][i]);
  }
 else
