...
int i,j,k,linenum,maxid,statenum,binsize,**data,**contactmatrix,**contactbool,**visitbool,endbool,**tempidlist,bordernum,*assign,len;
double **distmatrix;
...
printf("Input the length of the chromosome:\n");
scanf("%d",&len);
...
fclose(inputfile);
if(statenum*binsize*1000 > len)
 {
 printf("Error for the chromosome len:\n");
 exit(0);
 }

statenum = len/(binsize*1000)+1;
//statenum += 1;
