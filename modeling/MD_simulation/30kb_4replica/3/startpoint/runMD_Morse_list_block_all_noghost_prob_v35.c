...
//    forcevalue = (2/dist-0.667+20/(1+exp(-4*(dist-2))))*factor;
   ...
   for(k=0;k<DIM;k++)
    tempvector[k] /= dist;
   if(dist < 1.49)
    forcevalue = (2/dist-contactbool[i][j]*1.5/(2.25-dist*dist))*factor;
   else
...
int main()
{
char coorfilename[N],outputfilename[N],distmatrixfilename[N],contactboolfilename[N],comparefilename[N],coeffilename[N],assignfilename[N];
int i,j,k,l,m,statenum,**contactbool,stepnum,saveinterval,freq,coorfreq,shrinkfreq,seed,*assign,endbool,**neighborlist,*neighbornum,neigfreq,shrinkmax,maxlen;
double **coor,**velvector,**velvector2,**newcoor,radius,factor,bondfactor,**forcelist,bondlen,dt,sigma,kT,**originalmatrix,**vectorlist_neig,*distlist_neig,tempfactor,Fmax,dtp,t,distsq,coorfactor_record,darkprob,alpha,beta,fardist;
FILE *coorfile,*outputfile,*distmatrixfile,*contactboolfile,*comparefile,*coeffile,*assignfile;

printf("Input filename for initial coordinate:\n");
scanf("%s",coorfilename);

//printf("Input filename for initial distance matrix:\n");
//scanf("%s",distmatrixfilename);
//
...
//scanf("%d",&statenum);
//
printf("Input the number of steps for intergration:\n");
scanf("%d",&stepnum);
...
//printf("Input filename for alpha/beta coef:\n");
//scanf("%s",coeffilename);
//
//printf("Input the probability to consider dark interaction:\n");
//scanf("%lf",&darkprob);

printf("Input filename for output:\n");
scanf("%s",outputfilename);

printf("Input filename for compare:\n");
...
//newcoor = doublematrix(statenum,DIM);
forcelist = doublematrix(statenum,DIM);
//vectorlist_neig = doublematrix(statenum,DIM);
//distlist_neig = doublearray(statenum);
//slopematrix = doublematrix(statenum,statenum);
assign = intarray(statenum);
//neighborlist = intmatrix(statenum,statenum);
neighborlist = intmatrix(statenum,statenum);
...
//distmatrixfile = openfile(distmatrixfilename,'r');
contactboolfile = openfile(contactboolfilename,'r');
...
for(i=0;i<statenum;i++)
 ...
 for(j=0;j<statenum;j++)
  {
//  fscanf(distmatrixfile,"%lf",&distmatrix[i][j]);
  fscanf(contactboolfile,"%d",&contactbool[i][j]);
//  slopematrix[i][j] = tempfactor*pow(distmatrix[i][j],-TEMPPOW);
  }
 }
//fclose(distmatrixfile);
fclose(contactboolfile);
fclose(assignfile);

//coeffile = openfile(coeffilename,'r');
//fscanf(coeffile,"%lf",&alpha);
//fscanf(coeffile,"%lf",&beta);
//fclose(coeffile);
...
for(i=0;i<stepnum;i++)
 {
// factor = i*1.0/stepnum;
// factor = i*2.0/stepnum;
// if(factor > 1.0)
// factor = 0.5;
 ...
 if(i != 0)
  {
//  if((i%shrinkfreq == 0)&&(endbool == 0))
  if((i%shrinkfreq == 0)&&(i<shrinkmax))
//  if((i%shrinkfreq == 0))
   {
//   shrinkcoor(coor,distmatrix,contactbool,statenum);
   endbool=shrinkcoor_coor(coor,contactbool,statenum);
//   printf("%d %lf %lf\n",i,dt,t);
   }
////  if((i%coorfreq == 0)&&(i>2000))
//  if((i%coorfreq == 0))
//   updatedistmatrix_slope(coor,distmatrix,contactbool,statenum,slopematrix,tempfactor);
////   updatedistmatrix(coor,distmatrix,contactbool,statenum);
  }



// genvectordistlist(coor,statenum,vectorlist,id1list,id2list,distlist,listnum,vectorlist_neig,distlist_neig);
// if(i%100 > 10)
//  Fmax = getforcelist_Morsetargetdist_darkprob(coor,statenum,distmatrix,slopematrix,contactbool,forcelist,bondfactor,bondlen,factor,darkprob);	//De=kT,a=2/re,dr=re
//  Fmax = getforcelist_Morsetargetdist_darkprob_nobond(coor,statenum,distmatrix,slopematrix,contactbool,forcelist,bondfactor,bondlen,factor,darkprob);	//De=kT,a=2/re,dr=re
//  Fmax = getforcelist_Morsetargetdist_darkprob_compact(coor,statenum,distmatrix,slopematrix,contactbool,forcelist,bondfactor,bondlen,factor,darkprob);	//De=kT,a=2/re,dr=re
  if(i< shrinkmax)
   ...
   if((i-shrinkmax)%neigfreq == 0)
    {
    getneighborlist(coor,statenum,assign,contactbool,neighborlist,neighbornum,fardist);
    maxlen = 0;
    for(j=0;j<statenum;j++)
     {
   ...
   factor = (i-shrinkmax)*1.0/(stepnum-shrinkmax);
   Fmax = getforcelist_Morsetargetdist_darkprob_probforce_neighborlist(coor,statenum,assign,contactbool,forcelist,bondfactor,bondlen,factor,neighborlist,neighbornum);	//De=kT,a=2/re,dr=re
   }
//printf("%e\n",Fmax);
// else
//  Fmax = getforcelist_Morsetargetdist_darkprob_cross(coor,statenum,distmatrix,slopematrix,contactbool,forcelist,bondfactor,bondlen,factor,darkprob);	//De=kT,a=2/re,dr=re
...
comparefile = openfile(comparefilename,'w');
for(i=0;i<statenum;i++)
 for(j=i+1;j<statenum;j++)
  {
  distsq = getdistsq(coor[i],coor[j],DIM);
  distsq = sqrt(distsq);
  fprintf(comparefile,"%d %d %d %lf\n",i,j,contactbool[i][j],distsq);
  }
for(i=0;i<statenum;i++)
 {
 fprintf(comparefile,"Force: %e %e %e\n",forcelist[i][0],forcelist[i][1],forcelist[i][2]);
 }
fclose(comparefile);
