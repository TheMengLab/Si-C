...
int main()
{
char coorfilename[N],outputfilename[N],distmatrixfilename[N],contactboolfilename[N],comparefilename[N],coeffilename[N],assignfilename[N],targetfilename[N];
int i,j,k,l,m,statenum,**contactbool,stepnum,saveinterval,freq,coorfreq,shrinkfreq,seed,*assign,endbool,**neighborlist,*neighbornum,neigfreq,shrinkmax,maxlen,**targetlist,targetlen,**targetlist_array,*targetlist_len,**targetcontact;
...
FILE *coorfile,*outputfile,*distmatrixfile,*contactboolfile,*comparefile,*coeffile,*assignfile,*targetfile;
...
targetlen = getlinenum(targetfilename);
...
//neighborlist = intpointarray(statenum);
...
for(i=0;i<targetlen;i++)
 {
// printf("%d %d %d\n",targetlist[i][0],targetlist_len[targetlist[i][0]],targetlist[i][1]);
...
for(i=0;i<stepnum;i++)
 ...
 if(i != 0)
  ...
  if((i%shrinkfreq == 0)&&(i<shrinkmax))
   {
   endbool=shrinkcoor_coor_list(coor,targetlist,targetlen,statenum);
//   endbool=shrinkcoor_coor(coor,contactbool,statenum);
  ...
  if(i< shrinkmax)
   Fmax = getforcelist_Morsetargetdist_darkprob_probforce_list(coor,statenum,assign,targetlist,targetlen,forcelist,bondfactor,bondlen,factor);	//De=kT,a=2/re,dr=re
//   Fmax = getforcelist_Morsetargetdist_darkprob_probforce(coor,statenum,assign,contactbool,forcelist,bondfactor,bondlen,factor);	//De=kT,a=2/re,dr=re
   ...
   if((i-shrinkmax)%neigfreq == 0)
    {
    getneighborlist_list(coor,statenum,assign,targetlist_array,targetlist_len,neighborlist,neighbornum,fardist);
...
//   Fmax = getforcelist_Morsetargetdist_darkprob_probforce_neighborlist(coor,statenum,assign,contactbool,forcelist,bondfactor,bondlen,factor,neighborlist,neighbornum);	//De=kT,a=2/re,dr=re
   Fmax = getforcelist_Morsetargetdist_darkprob_probforce_neighborlist_list(coor,statenum,assign,targetlist,targetlen,forcelist,bondfactor,bondlen,factor,neighborlist,neighbornum);	//De=kT,a=2/re,dr=re
   }
